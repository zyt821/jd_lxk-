...
[task_local]
#东东世界
50 1 * * * jd_work_ddworld.js, tag= 东东世界
================Loon==============
[Script]
cron "50 1 * * *" script-path= jd_work_ddworld.js,tag= 东东世界
===============Surge=================
东东世界 = type=cron,cronexp="50 1 * * *",wake-system=1,timeout=3600,script-path= jd_work_ddworld.js
============小火箭=========
东东世界 = type=cron,script-path= jd_work_ddworld.js, cronexpr="50 1 * * *", timeout=3600, enable=true
*/
const $ = new Env('东东世界')
...
function PostRequest(uri,body) {
    const url = `https://ddsj-dz.isvjcloud.com/dd-api/do_task`;
    const method = `POST`;
    const headers = {"Accept": "application/json, text/plain, */*","Accept-Encoding": "gzip, deflate, br","Accept-Language": "zh-cn","Authorization": "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..9lYLh0Be3JaCIwY3kGniKidVq8TjYF7L1fElTtGASlg","Connection": "keep-alive","Content-Type": "application/x-www-form-urlencoded","Cookie": cookie,"Host": "ddsj-dz.isvjcloud.com","User-Agent": "jdapp;iPhone;10.1.4;14.4;0bcbcdb2a68f16cf9c9ad7c9b944fd141646a849;network/4g;model/iPhone12,1;addressid/5488734967;appBuild/167814;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 14_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1"}
    return {url: url, method: method, headers: headers, body: body};
}

function GetRequest(uri) {
    const url = `https://ddsj-dz.isvjcloud.com/dd-api/get_task`;
    const method = `GET`;
    const headers = {"Accept": "application/json, text/plain, */*","Accept-Encoding": "gzip, deflate, br","Accept-Language": "zh-cn","Authorization": "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..9lYLh0Be3JaCIwY3kGniKidVq8TjYF7L1fElTtGASlg","Connection": "keep-alive","Content-Type": "application/x-www-form-urlencoded","Cookie": cookie,"Host": "ddsj-dz.isvjcloud.com","User-Agent": "jdapp;iPhone;10.1.4;14.4;0bcbcdb2a68f16cf9c9ad7c9b944fd141646a849;network/4g;model/iPhone12,1;addressid/5488734967;appBuild/167814;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 14_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1"}
...
async function doTask(){
    const body = `taskToken=${token}&task_id=${taskid}`
        ...
        $.get(MyRequest,async(error, response, data) =>{
            try{
                ...
                if(result.bizCode == 0){
                    ...
                    let list1 = result.result.taskVos.find(item => item.taskId == 1)
                    list0tokenArr.push(1+list1.simpleRecordInfoVo.taskToken)
                    list1tokenArr.push(list1.simpleRecordInfoVo.taskToken)

                    let list2 = result.result.taskVos.find(item => item.taskId == 2)
                    for(let i = 0; i < list2.browseShopVo.length; i ++){
                        listtokenArr.push(2+list2.browseShopVo[i].taskToken)
                        list2tokenArr.push(list2.browseShopVo[i].taskToken)

                    }

                    let list3 = result.result.taskVos.find(item => item.taskId == 3)
                    ...
                    let list4 = result.result.taskVos.find(item => item.taskId == 4)
        ...
        $.get(MyRequest,async(error, response, data) =>{
            try{
                ...
                if(result.bizCode == 0){
                    ...
                    let list5 = result.result.taskVos.find(item => item.taskId == 5)
    ...
    return new Promise((resolve) => {
        $.get(MyRequest,async(error, response, data) =>{
            try{
                ...
                if(logs)$.log(data)
                if(result.bizCode == 0){
                    let userScore = result.result.userScore
                    if(userScore < 200){
                        $.log("共有金币："+userScore+";不足以兑换")
                    }
                    else{
                        $.msg("共有金币："+userScore+";请立即兑换：https://ddsj-dz.isvjcloud.com/dd-world/load_app/load_app.html")
                    }
                }else{
                    $.log(result.bizMsg+"\n")
                }
