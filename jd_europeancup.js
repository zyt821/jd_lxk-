...
Wenmoux
助力逻辑：
号1默认给温某人大佬助力，号2默认给zero205助力 后续号给号1助力
...
$.shareuuid = "" //原作者温某人大佬的助力码 
$.shareuuid2 = "" //zero205的助力码 
!(async () => {
    if (!cookiesArr[0]) {
...
function getToken() {
    let config = {
        url: 'https://api.m.jd.com/client.action?functionId=genToken&clientVersion=9.4.6&build=87373&client=android&uuid=69be0000ae5409c4&st=1625461998443&sign=5bd35533a73eeda60d4037efb2838d72&sv=102',
        body: 'body=%7B%22action%22%3A%22to%22%2C%22to%22%3A%22https%253A%252F%252Flzdz-isv.isvjcloud.com%252Fdingzhi%252Fhisense%252Feuropeancup%252Factivity%253FactivityId%253D901100032442101%2526hideFoot%253D1%22%7D&',
        headers: {
            'Host': 'api.m.jd.com',
            'accept': '*/*',
...
function getActCk() {
    return new Promise(resolve => {
        $.get(taskUrl("/dingzhi/hisense/europeancup/activity", `activityId=${$.actid}&shareUuid=&adsource=null&initHash=/home&shareuserid4minipg=8A%2BMf3SBYE8spQtvzQ2VLE7oeVP9kq2pYSH90mYt4m3fwcJlClpxrfmVYaGKuquQkdK3rLBQpEQH9V4tdrrh0w%3D%3D&shopid=undefined&lng=107.146945&lat=33.255267&sid=cad74d1c843bd47422ae20cadf6fe5aw&un_area=8_573_6627_52446`), (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function getUid() {
    return new Promise(resolve => {
        let body = `activityId=${$.actid}&pin=${encodeURIComponent($.pin)}&pinImg=${$.pinImg}&nick=${encodeURIComponent($.nick)}&cjyxPin=&cjhyPin=&shareUuid=&picId=`
...
function getinfo() {
    ...
    return new Promise(resolve => {
        $.post(taskPostUrl('/dingzhi/hisense/europeancup/myInfo', body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
function dotask(taskId, params) {
    let config = taskUrl("/dingzhi/hisense/europeancup/doTask", `taskId=${taskId}&${params ? ("param=" + params + "&") : ""}activityId=${$.actid}&pin=${encodeURIComponent($.pin)}&actorUuid=${$.uuid}&userUuid=`)
    //     console.log(config)
    return new Promise(resolve => {
        $.get(config, async (err, resp, data) => {
            try {
...
function taskUrl(url, body) {
    ...
    return {
        ...
        headers: {
            ...
            'Referer': `https://lzdz-isv.isvjcloud.com/dingzhi/hisense/europeancup/activity/7431935?activityId=901100032442101&shareUuid=&adsource=null&initHash=/home&shareuserid4minipg=8A%2BMf3SBYE8spQtvzQ2VLE7oeVP9kq2pYSH90mYt4m3fwcJlClpxrfmVYaGKuquQkdK3rLBQpEQH9V4tdrrh0w%3D%3D&shopid=undefined&lng=107.146945&lat=33.255267&sid=cad74d1c843bd47422ae20cadf6fe5aw&un_area=8_573_6627_52446`,
